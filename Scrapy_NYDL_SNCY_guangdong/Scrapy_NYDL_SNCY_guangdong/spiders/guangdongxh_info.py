from Scrapy_NYDL_SNCY_guangdong import upload_replace as ur
from Scrapy_NYDL_SNCY_guangdong.pipelines import MongoDBPipeline
from Scrapy_NYDL_SNCY_guangdong.items import InfoItem  # 需要修改下爬虫项目名！！！
...
class GuangdongxhInfoSpider(scrapy.Spider):
    name = 'guangdongxh_info'
    # allowed_domains = ['w']
    start_urls = ["http://www.gdwha.org/News/tongzhigonggao.html",
                  'http://www.gdwha.org/News/xiehuixinwen.html',
                  "http://www.gdwha.org/Industry/"
                  ]
    web_name = '广东省水利水电行业协会'  # 网站名称
    category = '能源电力'  # 模块
    ...
    def parse(self, response):
        ...
        #     next_page = "http://www.gdwha.org/News/tongzhigonggao_{0}.html".format(num)
        #     yield scrapy.Request(url=next_page, callback=self.parse)
        # for num1 in range(2, 9):
        #     next_page1 = "http://www.gdwha.org/News/xiehuixinwen_{0}.html".format(num1)
        #     yield scrapy.Request(url=next_page1, callback=self.parse)
        #
        # for num2 in range(2, 3):
        #     next_page2 = "http://www.gdwha.org/Industry/index_{0}.html".format(num2)
    ...
    def detail_parse(self, response):
        ...
        # 固定模式，改下xpath即可
        content = response.xpath('//div[@class="show_content"]').extract()
        content = ''.join(content)  # 转化成字符串，并且去掉空格

        # 处理图片的通用格式:
        _xpath1 = '//div[@class="show_content"]//img/@src'  # 改下xpath即可
        images = response.xpath(_xpath1).extract()
        # 处理附件的通用格式
        _xpath2 = '//div[@class="articleDes"]//a/@href'  # 改下xpath即可
...
if __name__ == '__main__':
    os.system('scrapy crawl guangdongxh_info')  ##改下爬虫名
