from Scrapy_NYDL_SNCY_liaoning import upload_replace as ur
from Scrapy_NYDL_SNCY_liaoning.pipelines import MongoDBPipeline
from Scrapy_NYDL_SNCY_liaoning.items import InfoItem  # 需要修改下爬虫项目名！！！
...
class LiaoningxhInfoSpider(scrapy.Spider):
    name = 'liaoningxh_info'
    # allowed_domains = ['w']
    start_urls = ['http://www.lnslxh.org/xhdt/index.htm']
    web_name = '辽宁省水利学会'  # 网站名称
    category = '能源电力'  # 模块
    ...
    def parse(self, response):
        a_list = response.xpath('//a[@class="a4"]')
        span_list = response.xpath('//span[@class="fonthui"]')
        for a,span in zip(a_list,span_list):
            ...
            issue_time = span.xpath('./text()').extract_first()
        ...
        #     next_page = "http://www.lnslxh.org/xhdt/index_{0}.htm".format(num)
    ...
    def detail_parse(self, response):
        ...
        # 处理图片的通用格式:
        _xpath1 = '//td[@class="fonthei"]//img/@src'  # 改下xpath即可
...
if __name__ == '__main__':
    os.system('scrapy crawl liaoningxh_info')  ##改下爬虫名
