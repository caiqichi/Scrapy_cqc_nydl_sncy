from Scrapy_NYDL_SNCY_zhejiang import upload_replace as ur
from Scrapy_NYDL_SNCY_zhejiang.pipelines import MongoDBPipeline
from Scrapy_NYDL_SNCY_zhejiang.items import InfoItem  # 需要修改下爬虫项目名！！！
...
class ZhejiangxhInfoSpider(scrapy.Spider):
    name = 'zhejiangxh_info'
    # allowed_domains = ['w']
    start_urls = ['http://www.zsjhx.com/news.asp?clsid=15',
                  'http://www.zsjhx.com/news.asp?clsid=46']
    web_name = '浙江省水利建设行业协会'  # 网站名称
    category = '能源电力'  # 模块
    ...
    def parse(self, response):
        li_list = response.xpath('//div[@class="kuang"]/ul[@class="kuang_list1"]/li')

        for li in li_list:
            content_url = li.xpath("./a/@href").extract_first()
            content_url = response.urljoin(content_url)
            title = li.xpath('./a/text() | ./a/span/@title').extract_first()
            issue_time = li.xpath('./p/text()').extract_first()
            import re
        ...
        #     next_page = "http://www.zsjhx.com/news.asp?pid=0&clsid=15&keyword=&page={0}".format(num)
    ...
    def detail_parse(self, response):
        ...
        # 固定模式，改下xpath即可
        content = response.xpath('//div[@class="kuang"]').extract()
        content = ''.join(content)  # 转化成字符串，并且去掉空格

        # 处理图片的通用格式:
        _xpath1 = '//div[@id="news_content"]//img/@src'  # 改下xpath即可
        images = response.xpath(_xpath1).extract()
        # 处理附件的通用格式
        _xpath2 = '//div[@id="news_content"]//a/@href'  # 改下xpath即可
        ...
        if "clsid=15" in item["content_url"]:
...
if __name__ == '__main__':
    os.system('scrapy crawl zhejiangxh_info')  ##改下爬虫名
