from Scrapy_CQC_NYDL_SNCY_beijingbm_zc import upload_replace as ur
from Scrapy_CQC_NYDL_SNCY_beijingbm_zc.pipelines import MongoDBPipeline
from Scrapy_CQC_NYDL_SNCY_beijingbm_zc.items import InfoItem  # 需要修改下爬虫项目名！！！
...
class BeijingbmZcSpider(scrapy.Spider):
    name = 'beijingbm_zc'
    # allowed_domains = ['ww']
    start_urls = ['http://swj.beijing.gov.cn/zwgk/zcfg/zxfg/']
    web_name = '北京市水务局'  # 网站名称
    category = '能源电力'  # 模块
    ...
    def parse(self, response):
        ...
        #     next_page = "http://swj.beijing.gov.cn/zwgk/zcfg/zxfg/index_{0}.html".format(num)
    ...
    def detail_parse(self, response):
        ...
        # 固定模式，改下xpath即可
        content = response.xpath('//div[@class="swj_main"]').extract()
        ...
        # 处理附件的通用格式
        _xpath2 = '//div[@id="mainText"]//a/@href'  # 改下xpath即可
...
if __name__ == '__main__':
    os.system('scrapy crawl beijingbm_zc')  ##改下爬虫名
