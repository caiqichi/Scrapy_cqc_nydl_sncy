from Scrapy_NYDL_SNCY_qinghai import upload_replace as ur
from Scrapy_NYDL_SNCY_qinghai.pipelines import MongoDBPipeline
from Scrapy_NYDL_SNCY_qinghai.items import InfoItem  # 需要修改下爬虫项目名！！！
...
class QinghaixhInfoSpider(scrapy.Spider):
    name = 'qinghaixh_info'
    # allowed_domains = ['w']
    start_urls = ['http://www.qhwea.cn/article/xyyw',
                  "http://www.qhwea.cn/article/gzdt",
                  "http://www.qhwea.cn/article/zffg"]
    web_name = '青海省水利工程协会'  # 网站名称
    category = '能源电力'  # 模块
    ...
    def parse(self, response):
        li_list = response.xpath('//div[@class="weui-pl20 weui-pr20 weui-pt10 weui-pb10"]/ul[@class="main_ul"]/li')
        ...
        #     next_page = "http://www.qhwea.cn/article/xyyw/page/{0}".format(num)
        #     yield scrapy.Request(url=next_page, callback=self.parse)
        #
        # for num1 in range(2, 3):
        #     next_page1 = "http://www.qhwea.cn/article/gzdt/page/{0}".format(num1)
        #     yield scrapy.Request(url=next_page1, callback=self.parse)
        #
        # for num2 in range(2, 3):
        #     next_page2 = "http://www.qhwea.cn/article/zffg/page/{0}".format(num2)
    ...
    def detail_parse(self, response):
        ...
        # 固定模式，改下xpath即可
        content = response.xpath('//div[@class="weui-pl30 weui-pr30 weui-pt10 item"]').extract()
...
if __name__ == '__main__':
    os.system('scrapy crawl qinghaixh_info')  ##改下爬虫名
