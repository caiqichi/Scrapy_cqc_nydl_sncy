...
from Scrapy_CQC_NYDL_SNCY_hangyebiaozhun.items import InfoItem
from Scrapy_CQC_NYDL_SNCY_hangyebiaozhun.pipelines import MongoDBPipeline
from Scrapy_CQC_NYDL_SNCY_hangyebiaozhun import upload_replace as ur
...
class HangyebiaozhunSpider(scrapy.Spider):
    name = 'hangyebiaozhun'
    # allowed_domains = ['ww']
    # start_urls = ['http://ww/']
    web_name = '行业标准信息服务平台'
    ...
    def start_requests(self):
        # for i in range(1, 16):
        for i in range(1, 2):
            url = 'http://hbba.sacinfo.org.cn/stdQueryList'
    ...
    def parse(self, response):
        ...
        for i in records:
            ...
            state = i['status']
            id = i['pk']  # 详情页需要
            content_url = "http://hbba.sacinfo.org.cn/stdDetail/{0}".format(id)
            project_id = None
...
if __name__ == "__main__":
    os.system("scrapy crawl hangyebiaozhun")
