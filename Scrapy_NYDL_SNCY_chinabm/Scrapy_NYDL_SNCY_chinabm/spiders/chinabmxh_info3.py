...
class ChinabmxhInfo3Spider(scrapy.Spider):
    name = 'chinabmxh_info3'
    # allowed_domains = ['w']
    start_urls = ['http://f.mnr.gov.cn/index_3553.html',
                  ]
    web_name = '中华人民共和国自然资源部'  # 网站名称
    category = '能源电力'  # 模块
    ...
    def parse(self, response):
        li_list = response.xpath('//div[@class="dasite"]/table//td/a')
        issue_time_list = response.xpath('//div[@class="dasite"]/table/tr[2]/td[4]')

        for li,issue_time in zip(li_list,issue_time_list):
            content_url = li.xpath("./@href").extract_first()
            content_url = response.urljoin(content_url)
            title = li.xpath('./text()').extract_first()
            try:
                issue_time = issue_time.xpath('./text()').extract_first()
        ...
        #     next_page = "http://f.mnr.gov.cn/index_3553_{0}.html".format(num)
...
if __name__ == '__main__':
    os.system('scrapy crawl chinabmxh_info3')  ##改下爬虫名
